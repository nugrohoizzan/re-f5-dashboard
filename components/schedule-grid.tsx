    ...
    <div className="max-h-[70vh] overflow-auto">
      <table className="sched-table w-full text-sm">
            ...
            {days.map((d) => {
              ...
              return (
                <th
                  key={d}
                  className={cn(
                    "sched-sticky-head min-w-[64px] px-2 py-2 text-center text-xs font-medium",
                    weekend && "bg-zinc-800"
                  )}
                >
                  <div>{format(date, "EEE")}</div>
                  <div className="text-zinc-300">{format(date, "d/M")}</div>
                </th>
              );
            })}
          </tr>
        ...
        <tbody>
          {engineers.map((eng) => (
            <tr key={eng.id}>
              <td className="sched-sticky-col px-3 py-1.5 text-sm font-medium text-zinc-900">
                {eng.displayName}
              </td>
              {days.map((d) => {
